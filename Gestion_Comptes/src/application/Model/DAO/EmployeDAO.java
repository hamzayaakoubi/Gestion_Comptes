			...
			ResultSet res=	req.executeQuery();
				while (res.next())
				{
					...
					for (int i=0; rs.next(); i++)
					{
						...
						Client clt =new Client(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7) );
	...
	//Update 
			public void update (Employe emp) {		
				...
				{	
					int n=0;
					try {
						PreparedStatement req=cnx.prepareStatement("update employe Set Nom=?, Prenom=?,Adresse=?, DN=?, Email=?,NumeroTel=?  where NumeroEmploye=?");
						...
						req.setString(7, emp.getNumeroEmploye());
